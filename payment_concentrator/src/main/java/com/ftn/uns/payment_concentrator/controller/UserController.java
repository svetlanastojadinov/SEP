...
public class UserController {
	...
	@RequestMapping(value = "/getBoughtMagazines", method = RequestMethod.GET)
	public ResponseEntity<List<Magazine>> getBoughtMagazines() {
		List<Magazine> boughtMagazines = new ArrayList<Magazine>();

		User user = userService.findByUsername(SecurityContextHolder.getContext().getAuthentication().getName());
		List<Order> allOrdersByUser = orderService.findAllByBuyer(user.getUsername());
		if (allOrdersByUser != null) {
			for (Order order : allOrdersByUser) {
				if (order.getMagazine() != null && order.getExecuted() == true) {
					boughtMagazines.add(order.getMagazine());
					removeMagazineFromCart(order.getMagazine().getIssn());
				}
			}
		}
		return new ResponseEntity<>(boughtMagazines, HttpStatus.OK);
	}

	@RequestMapping(value = "/getBoughtArticles", method = RequestMethod.GET)
	public ResponseEntity<List<Article>> getBoughtArticles() {
		...
		if (allOrdersByUser != null) {
			for (Order order : allOrdersByUser) {
				if (order.getArticle() != null && order.getExecuted() == true) {
					boughtArticles.add(order.getArticle());
					removeArticleFromCart(order.getArticle().getId());
				}
			}
		}
		return new ResponseEntity<>(boughtArticles, HttpStatus.OK);
	}
